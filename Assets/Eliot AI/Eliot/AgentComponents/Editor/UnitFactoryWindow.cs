...
namespace Eliot.AgentComponents.Editor
{
    ...
    public class UnitFactoryWindow : EditorWindow
    {
        ...
        private bool _useIntoxication;
        /// Maximum intoxication capacity.
        private int _maxIntoxication = 100;
        /// Wheather the Agent uses morale as a resource.
        private bool _useMorale;
        /// Maximum morale capacity.
        private int _maxMorale = 100;
        /// Wheather the Agent uses temperament as a resource.
        private bool _useTemperament;
        /// Maximum temperament capacity.
        private int _maxTemperament = 100;
        /// Agent's graphics.
        ...
        private void OnGUI()
        {
            ...
            _useIntoxication = EditorGUILayout.Toggle("Does it use intoxication?", _useIntoxication);
            if (_useIntoxication)
                _maxIntoxication = EditorGUILayout.IntField("Intoxication capacity: ", _maxIntoxication);
            _useMorale = EditorGUILayout.Toggle("Does it use morale?", _useMorale);
            if (_useMorale)
                _maxMorale = EditorGUILayout.IntField("Morale capacity: ", _maxMorale);
            _useTemperament = EditorGUILayout.Toggle("Does it use temperament?", _useTemperament);
            if (_useTemperament)
                _maxTemperament = EditorGUILayout.IntField("Temperament capacity: ", _maxTemperament);
            Space(2);
            Header("Other");
